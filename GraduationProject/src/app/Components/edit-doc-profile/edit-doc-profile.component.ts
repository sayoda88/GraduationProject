...
const URL = 'https://mazen.cyclic.app/doctor/upload';
...
export class EditDocProfileComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this._DoctorService.getProfileDoc().subscribe(this.observerForEditProfileDoc);
  ...
  editProfDoc(form: any) {
    ...
    this._DoctorService.editDocProfile(form.value).subscribe((data)=>{if(data.message=='Done'){ console.log(data);this.msgOfUpdate();this.ngOnInit;}});
  }
  msgOfUpdate(){
